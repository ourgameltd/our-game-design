...
import { getReportsByClubId } from '@/data/reports';
...
export default function ClubReportCardsPage() {
  ...
  const reports = clubId ? getReportsByClubId(clubId) : [];
  ...
  })).filter(item => item.player); // Filter out any reports without players
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <main className="mx-auto px-4 py-4">
        ...
        <PageTitle 
          title="Report Cards"
          subtitle={`All player report cards across ${club.name}`}
        />
